...
(** An input channel reading from the bytes buffer. See {!of_string} for more
    details. *)

val input : #t -> bytes -> int -> int -> int
(** Read bytes into the given buffer. This returns [0] only if the stream has
    reached its end.
...
(** [really_input_string ic n] reads exactly [n] bytes of [ic] and returns them
    as a string.
    @raise End_of_file if the input does not contain enough data.
    @since 0.2 *)
