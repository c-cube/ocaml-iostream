...
(** [decompress_in ic] returns a new input stream that is the decompressed
    version of [ic] *)
...
(** [compress_in ?level ic] is a new input stream that is the compressed version
    of [ic].
...
(** [compressed_out oc] takes a output stream [oc], and returns a new output
    stream [oc2]. Writing some (normal) bytes to [oc2] will write a compressed
    version of these bytes into [oc] (possibly after a flush). *)
...
(** [decompressed_out oc] is a new output stream [oc2]. Writing (compressed)
    bytes to [oc2] will write their decompressed version into [oc] (possibly
    after a flush) *)
